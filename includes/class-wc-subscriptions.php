...
class At_WC_Gateway_Atara_Subscription extends At_WC_Atara_Gateway
{
    ...
    /**
     * Don't transfer Atara fee/ID meta to renewal orders.
     *
    ...
    public function delete_renewal_meta( $renewal_order )
    {
        ...
        delete_post_meta($order_id, '_atara_fee');
        delete_post_meta($order_id, '_atara_net');
        delete_post_meta($order_id, '_atara_currency');
    ...
    public function scheduled_subscription_payment( $amount_to_charge, $renewal_order )
    {
        ...
        if (is_wp_error($response) ) {
            ...
            $renewal_order->update_status('failed', sprintf('Atara Transaction Failed (%s)', $response->get_error_message()));
    ...
    public function process_subscription_payment( $order = '', $amount = 0 )
    {
        ...
        $auth_code = get_post_meta($order_id, '_tbz_atara_wc_token', true);
        ...
        if ($auth_code ) {
            ...
            if (! is_wp_error($request) && 200 == wp_remote_retrieve_response_code($request) ) {
                ...
                if ('success' === $status && in_array($response_code, $valid_response_code) ) {
                    ...
                    $message = ( sprintf('Payment via Atara successful (<strong>Transaction Reference:</strong> %s | <strong>Payment Reference:</strong> %s)', $txn_ref, $payment_ref) );
                    ...
                    return new WP_Error('atara_error', 'Atara payment failed. ' . $response->message);
        ...
        return new WP_Error('atara_error', 'This subscription can\'t be renewed automatically. The customer will have to login to his account to renew his subscription');
