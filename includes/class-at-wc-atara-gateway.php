...
class At_WC_Atara_Gateway extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        $this->live_tokenized_url   =  AT_WC_ATARA_API_LIVE_URL . '/aptokenized/charge';
        $this->tokenized_url        = $this->testmode ? $this->test_tokenized_url : $this->live_tokenized_url;
        $this->test_exchange_url      = AT_WC_ATARA_API_SANDBOX_URL . '/api/abokiExchangeRate';
        $this->live_exchange_url      = AT_WC_ATARA_API_LIVE_URL . '/api/abokiExchangeRate';
        $this->exchange_url        = $this->testmode ? $this->test_exchange_url : $this->live_exchange_url;
        $this->allow_service_provider_commission = $this->get_option('allow_service_provider_commission') === 'yes' ? true : false;
    ...
    public function admin_options()
    {
        ...
        <br>
        <h4>Your Callback URL:<span><i style="color: #2271b1; padding-left:10px"> <?php echo get_bloginfo('wpurl') . "/wc-api/at_wc_atara_gateway"; ?></i></span></h4>
        <?php
        ...
        if ($this->is_valid_for_use()) {
            ...
            <div class="inline error">
                <p><strong>Atarapay Payment Plugin Disabled</strong>: <?php echo $this->msg ?></p>
            </div>
    ...
    public function init_form_fields()
    {
        ...
        'description'   => array(
        'title'       => __('Description', 'atarapay-woo'),
        'type'        => 'textarea',
        'description' => __('This controls the payment method description which the user sees during checkout.', 'atarapay-woo'),
        'default'     => 'When you make payment, it goes into a secure holding account. The seller is informed and delivers to you. Then inspect your order before releasing the payment to the seller. All payments made are insured by our insurance partners to protect all parties; buyer, seller, and service provider. Learn more about atarapay <a href="https://www.atarapay.com" target="_blank">here</a>.',
        'desc_tip'    => true,
        ),
        'testmode'              => array(
        'title'       => 'Test mode',
        ...
        'allow_service_provider_commission' => array(
        'title'       => 'Split Payment',
        'label'       => 'Allow Service Provider Payout',
        'type'        => 'checkbox',
        'default'     => false,
        'desc_tip'    => true,
        ),
    ...
    public function payment_scripts()
    {
        ...
        WC_Order:$order = wc_get_order($order_id);
        ...
        wp_enqueue_script('at_wc_atara', plugins_url('assets/js/atara.js?v=2.0.2', AT_WC_ATARA_MAIN_FILE), array('jquery', 'at_atara'), AT_WC_ATARA_VERSION, false);
        ...
        if (is_checkout_pay_page() && get_query_var('order-pay')) {
            ...
            foreach ($order->get_items() as $item_id => $item) {
                ...
                $products[] = array(
                    ...
                    'product_type' => ! $product->is_virtual() ?  'physical' : 'virtual',
                    'shipping' => ! $product->is_virtual() ?  'yes' : 'no',
                );
            ...
            foreach($all_product_id as $get_product_id) {
                $all_service_providers[] = get_post_meta($get_product_id, '_at_wc_atara_service_provider', true) ? get_post_meta($get_product_id, '_at_wc_atara_service_provider', true) : null;
            }

            if(count(array_unique($all_service_providers)) != 1) {
                wc_add_notice('The cart can not contain products or services from more than one service provider', 'error');
            }else{
                add_action('woocommerce_receipt_' . $this->id, array($this, 'receipt_page'));
            }


            //adding service_provider_id
            $service_provider_id = null;
            $service_provider_commission = null;
            if ($this->allow_service_provider_commission) {
                ...
                $service_provider_commission = get_post_meta($product_id, '_at_wc_atara_service_provider_commission', true) ? get_post_meta($product_id, '_at_wc_atara_service_provider_commission', true) : 0;
                $seller_commission = 100 - $service_provider_commission;
            }
            ...
            if ($the_order_id == $order_id && $the_order_key == $order_key) {
                ...
                $atara_params['sp_id']                 = $service_provider_id;
                $atara_params['sp_commission']         = $seller_commission;
                $atara_params['hash']                = $this->generate_hash($atara_params);
                $atara_params['url']                 = get_site_url();
                $atara_params['products']            = $products;

                $atara_params = apply_filters( 'at_checkout_params', $atara_params, $order_id );
    ...
    {
        $request = wp_remote_get($this->exchange_url, array('timeout' => 60, 'body' => array('base' => $local, 'currency' => $currency)));
        ...
        if (!empty($data)) {
            $rate = $data->data->USDNGN;
            $amountFx = $rate*$amount;
            return $amountFx;
        }
    }
     ...
     */
    public function verify_atara_transaction() // no verification whatsoever, just completing payment. NEED FIX.
    {
        if (isset($_REQUEST['product'])) {
            ...
            update_post_meta($order_id, '_atara_status', At_WC_Atara_Order::PENDING_STATUS);
